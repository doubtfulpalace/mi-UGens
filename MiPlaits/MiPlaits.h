...
/*
 ...
 UGEN_NAME - a clone of the eurorack module 'Plaits' by
...
//static void Plaits_CtorBase(PlaitsUnit *unit);
//static void Plaits_DtorBase(PlaitsUnit *unit);
static void Plaits_next(PlaitsUnit *unit, int inNumSamples);

static void Plaits_CtorBase(PlaitsUnit *unit, int engine_num) {
    ...
    unit->voice_->Init(&allocator, engine_num);
    ...
    SETCALC(Plaits_next);
    //UGEN_NAME_next(unit, 64);       // do we reallly need this?
...
#define BUILD_PLAITS(engine, engine_num) \
typedef PlaitsUnit MiPlaits##engine; \
\
static void MiPlaits##engine##_Ctor(PlaitsUnit *unit) { \
	Plaits_CtorBase(unit, engine_num); \
} \
\
static void MiPlaits##engine##_Dtor(PlaitsUnit *unit) { \
	Plaits_DtorBase(unit); \
} \
\
PluginLoad(MiPlaits##engine) { \
    ft = inTable; \
    DefineDtorUnit(MiPlaits##engine); \
}\
;
